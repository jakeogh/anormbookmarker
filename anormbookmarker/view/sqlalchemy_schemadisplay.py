...
#    iostream = StringIO(create_uml_graph(*args, **kwargs).create_png())
    iostream = BytesIO(create_uml_graph(*args, **kwargs).create_png())
    Image.open(iostream).show(command=kwargs.get('command','feh'))
...
#    iostream     = StringIO(create_schema_graph(*args, **kwargs).create_png())
    iostream     = BytesIO(create_schema_graph(*args, **kwargs).create_png())
    Image.open(iostream).show(command=kwargs.get('command','feh'))
...
#    show_schema_graph(metadata=model.Base.metadata)
    show_uml_graph(mappers=mappers)
